...
cleaned AS (
    ...
    {% if is_incremental() %}
        WHERE CAST(step AS INT) > (SELECT MAX(step_hour) FROM {{ this }})
    {% endif %}
)
...
SELECT 
    ...
    *,
    ...
    CASE WHEN receiver_account_id LIKE 'M%' THEN TRUE ELSE FALSE END AS is_receiver_merchant,
    ...
    CASE WHEN sender_old_balance = sender_new_balance AND transaction_amount > 0 THEN TRUE ELSE FALSE END AS is_zero_impact_txn,
